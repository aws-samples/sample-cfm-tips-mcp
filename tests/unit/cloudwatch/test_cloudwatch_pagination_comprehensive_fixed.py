#!/usr/bin/env python3
"""
Comprehensive fixed unit tests for CloudWatch pagination functionality.
Tests all CloudWatch APIs that support pagination with proper mocking.
"""
...
class TestCloudWatchPaginationComprehensiveFixed:
    """Comprehensive fixed unit tests for all CloudWatch APIs that support pagination."""
    ...
    def create_query_response(self, items, page=1, items_per_page=10):
        """Create a mock query response with proper structure."""
        total_items = len(items)
        total_pages = (total_items + items_per_page - 1) // items_per_page
        start_idx = (page - 1) * items_per_page
        end_idx = start_idx + items_per_page
        ...
        return {
            ...
            'data': {
                'query_results': {
                    ...
                    'query_metadata': {
                        'sql_query': 'SELECT * FROM cloudwatch_analysis',
                        'execution_time_ms': 125.5,
                        'rows_examined': total_items,
                        'session_id': 'test-session-123'
                    }
                }
            }
    ...
    async def test_cloudwatch_comprehensive_optimization_pagination(self, mock_large_dataset):
        ...
        with patch('playbooks.cloudwatch.optimization_orchestrator.CloudWatchOptimizationOrchestrator') as mock_orchestrator_class:
            ...
            async def mock_execute_comprehensive_analysis(*args, **kwargs):
                page = kwargs.get('page', 1)
                return self.create_orchestrator_response(mock_large_dataset, page)
            
            mock_orchestrator.execute_comprehensive_analysis = AsyncMock(side_effect=mock_execute_comprehensive_analysis)
    ...
    async def test_cloudwatch_comprehensive_optimization_tool_pagination(self, mock_large_dataset):
        ...
        with patch('playbooks.cloudwatch.cloudwatch_optimization_tool.CloudWatchOptimizationTool') as mock_tool_class:
            mock_tool = MagicMock()
            mock_tool_class.return_value = mock_tool
            
            async def mock_execute_comprehensive_optimization_analysis(*args, **kwargs):
                page = kwargs.get('page', 1)
                return self.create_orchestrator_response(mock_large_dataset, page)
            
            mock_tool.execute_comprehensive_optimization_analysis = AsyncMock(side_effect=mock_execute_comprehensive_optimization_analysis)
    ...
    async def test_query_cloudwatch_analysis_results_pagination(self, mock_large_dataset):
        ...
        with patch('playbooks.cloudwatch.optimization_orchestrator.CloudWatchOptimizationOrchestrator') as mock_orchestrator_class:
            ...
            # Mock the get_analysis_results method that the query function actually calls
            mock_orchestrator.get_analysis_results.return_value = mock_large_dataset
            ...
            response_p1 = json.loads(result_p1[0].text)
            assert response_p1['pagination']['current_page'] == 1
            assert len(response_p1['results']) == 10
            assert response_p1['pagination']['total_items'] == 23
            assert response_p1['pagination']['total_pages'] == 3
    ...
    @pytest.mark.asyncio
    async def test_pagination_metadata_consistency(self, mock_large_dataset):
        """Test that pagination metadata is consistent across all CloudWatch APIs."""
        
        apis_to_test = [
            ('general_spend', 'run_cloudwatch_general_spend_analysis', 'CloudWatchOptimizationOrchestrator', 'execute_analysis'),
            ('metrics', 'run_cloudwatch_metrics_optimization', 'CloudWatchOptimizationOrchestrator', 'execute_analysis'),
            ('logs', 'run_cloudwatch_logs_optimization', 'CloudWatchOptimizationOrchestrator', 'execute_analysis'),
            ('alarms', 'run_cloudwatch_alarms_and_dashboards_optimization', 'CloudWatchOptimizationOrchestrator', 'execute_analysis'),
        ]
        
        for api_name, func_name, orchestrator_class, method_name in apis_to_test:
            with patch(f'playbooks.cloudwatch.optimization_orchestrator.{orchestrator_class}') as mock_orchestrator_class:
                mock_orchestrator = MagicMock()
                mock_orchestrator_class.return_value = mock_orchestrator
                ...
                setattr(mock_orchestrator, method_name, AsyncMock(side_effect=mock_method))
                
                # Import and call the function
                from runbook_functions import run_cloudwatch_general_spend_analysis, run_cloudwatch_metrics_optimization, run_cloudwatch_logs_optimization, run_cloudwatch_alarms_and_dashboards_optimization
                
                func_map = {
                    'run_cloudwatch_general_spend_analysis': run_cloudwatch_general_spend_analysis,
                    'run_cloudwatch_metrics_optimization': run_cloudwatch_metrics_optimization,
                    'run_cloudwatch_logs_optimization': run_cloudwatch_logs_optimization,
                    'run_cloudwatch_alarms_and_dashboards_optimization': run_cloudwatch_alarms_and_dashboards_optimization,
                }
                
                api_func = func_map[func_name]
